from evaluation.question_gen.llm import run_chat_completion
from evaluation.question_gen.prompts import comparison_of_authorities, compare_docs, specific_doc, status_of_tags, system_prompt, trends_over_tags
import pandas as pd
import os
import itertools
import random
...
def generate_trends_prompts(num):
    ...
    tt_tags_years = [[random.sample(all_tags, 1)[0], random.sample(years, 1)[0]] for i in range(2000)]
    tt_tags_authorities_1 = [[random.sample(all_tags, 1)[0], random.sample(all_specific_authorities, 1)[0]] for i in range(1000)]
    tt_tags_authorities_2 = [[random.sample(all_tags, 1)[0], random.sample(list(general_authorities.keys()), 1)[0]] for i in range(1000)]
    tt_authorities_years_1 = [[random.sample(all_specific_authorities, 1)[0], random.sample(years, 1)[0]] for i in range(1000)] 
    tt_authorities_years_2 = [[random.sample(list(general_authorities.keys()), 1)[0], random.sample(years, 1)[0]] for i in range(1000)]
    tt_tay_1 = [[random.sample(all_tags, 1)[0], random.sample(all_specific_authorities, 1)[0], random.sample(years, 1)[0]] for i in range(2000)]
    tt_tay_2 = [[random.sample(all_tags, 1)[0], random.sample(list(general_authorities.keys()), 1)[0], random.sample(years, 1)[0]] for i in range(2000)]
...
DOC_COMP = 500                                  
TAG_STATUS = 10   
TAG_TRENDS = 30 # 6 combinations of questions, 100 each
AUTHORITY_COMP = 10
...
for i, p in enumerate(prompts):
    ...
    if (i + 1) % 20 == 0:
        ...
        out.to_csv("./evaluation/question_gen/questions.csv", index=False)
