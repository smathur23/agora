...
if online:
    questions = pd.read_csv("evaluation/questions_only.csv")
    RAG = RAGPretrainedModel.from_index(".ragatouille/colbert/indexes/agora_index")
else:
    source_file = "evaluation/eval_questions_with_chunks.jsonl"

def retrieve_docs(query):
    results = RAG.search(query, k=20)
    return results

with open("evaluation/discarded_questions_online.txt" if online else "evaluation/discarded_questions_offline.txt", "r") as f:
    num_discarded = sum(1 for line in f)

with open("evaluation/manually_labeled_questions_online.jsonl" if online else "evaluation/manually_labeled_questions_offline.jsonl", "r") as f:
...
if online:
    ...
    """)
        if q != "":
            with open("evaluation/discarded_questions_online.txt", "a") as f:
                f.write(f"{idx}\n")
            continue
        ...
        with open("evaluation/manually_labeled_questions_online.jsonl", "a") as f:
...
""")
            if q != "":
                with open("evaluation/discarded_questions_offline.txt", "a") as f:
                    f.write(f"{i}\n")
                continue
            ...
            with open("evaluation/manually_labeled_questions_offline.jsonl", "a") as f:
                f.write(json.dumps({"question": row["question"], "positive_ids": relevant, "negative_ids": irrelevant}))
                f.write("\n")