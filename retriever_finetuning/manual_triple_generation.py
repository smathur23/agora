online = True # set to true if you haven't already retrieved chunks for the questions
...
if online:
    ...
    source_file = "retriever_finetuning/questions_with_chunks.jsonl"

def retrieve_docs(query):
    results = RAG.search(query, k=20)
    return results

with open("retriever_finetuning/discarded_questions_online.txt" if online else "retriever_finetuning/discarded_questions_offline.txt", "r") as f:
    num_discarded = sum(1 for line in f)

with open("retriever_finetuning/manually_labeled_questions_online.jsonl" if online else "retriever_finetuning/manually_labeled_questions_offline.jsonl", "r") as f:
...
if online:
    ...
    """)
        if q != "":
            with open("retriever_finetuning/discarded_questions_online.txt", "a") as f:
                f.write(f"{idx}\n")
            continue
        ...
        with open("retriever_finetuning/manually_labeled_questions_online.jsonl", "a") as f:
...
""")
            if q != "":
                with open("retriever_finetuning/discarded_questions_offline.txt", "a") as f:
                    f.write(f"{i}\n")
                continue
            ...
            with open("retriever_finetuning/manually_labeled_questions_offline.jsonl", "a") as f:
                f.write(json.dumps({"question": row["question"], "positive_ids": relevant, "negative_ids": irrelevant}))
                f.write("\n")